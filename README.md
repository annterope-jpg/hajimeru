...
Phase 12では、眠気、疲れ、頭の霧、身体の重さ、freezeを本人の言葉で分け、休息・時間変更・小さい一歩・相談目安を選べるようにしました。状態があるときは複雑な計画を初期表示せず、観察時の現地時刻とタイムゾーンを再計算せず残します。詳細は[状態・睡眠・時間帯の読み違いを減らす仕様](docs/STATE_CONTEXT_SUPPORT.md)に記載しています。

Phase 13では、嫌悪とは別に、開始前の候補・手順・準備・順番の多さを本人が任意で選べるようにしました。選んだ場合だけ判断を1つ減らす案を1件表示し、未回答や保留から推測しません。詳細は[努力コストと意思決定点を減らす仕様](docs/DECISION_FRICTION.md)に記載しています。

Phase 14では、大きな課題の地図に、本人が入力した今日の範囲・一区切り・保留・再開地点を短い境界メモとして反映し、作成後も変更または取消できるようにしました。空欄や旧記録は補完しません。詳細は[大きな課題の共同ロードマップ仕様](docs/COLLABORATIVE_ROADMAP.md)に記載しています。

Phase 15では、開始直後の変化が見えにくいと本人が答えた場合だけ、「少し助かった結果」または「少し進めている途中」の近い未来を最大3枠で置ける任意機能を追加しました。価値アンカー、小報酬、再開キューとは分け、自由記述は進行中フローの端末内メモだけに保存し、AI・同期・履歴・書き出しへ含めません。詳細は[未来の一場面（EFT）最小仕様](docs/EPISODIC_FUTURE_SCENE.md)に記載しています。

Phase 16では、一人、静かな同席、開始前後のひとことを同等に選べる任意の社会支援実験を追加しました。人が安心か圧力かを本人が判断し、定型文にタスク本文を含めず、アプリから送信・連絡先保存・既読確認を行いません。選択は進行中実験のメモリ内だけにあり、記録・同期・書き出しへ含めません。詳細は[本人主導の社会的随伴性](docs/SOCIAL_SUPPORT_EXPERIMENT.md)に記載しています。

Phase 17では、タイマー中に「戻る目印」「次の1動作」「切替の橋」を1つだけ表示できるようにしました。元の開始記録・計画・タイマー・ロードマップは変更せず、一時表示は保存・送信しません。詳細は[脱線・再開・切替の最小支援](docs/REENTRY_AND_TRANSITION_SUPPORT.md)に記載しています。

Phase 18では、履歴1件から「面談に持っていくメモ」を作れるようにしました。全項目を未選択から始め、本人が選択・編集した内容だけをプレビューし、同じ文章だけをOS共有画面へ渡します。共有先・送信結果・共有履歴は保存せず、一般のJSON/CSV書き出しとは分離しています。詳細は[本人主導の共有要約](docs/PERSON_LED_SHARE_SUMMARY.md)に記載しています。
