...
- 回答済み項目だけを採点し、課題側の仮説を最大2つ選択。低覚醒・freezeは現在状態として別枠表示
- 初回説明で「原因を決めない小さな実験」と本人の選択権を確認し、開始プランの仮説へ「近い・違う・まだ分からない」と任意で返答
...
- 最優先の課題側仮説に対応する30秒以内の最初の行動と理由、開始キュー、起動行動、妨害対策、小報酬
- 1・3・5分タイマーと、開始時点での成功記録
- 再起動後の実行中タイマー復元、途中終了を否定しない任意の振り返り
- SQLiteローカル保存、同日1件の状態記録、試した／残した一歩・工夫・状態の非評価的な振り返り
...
- 本人にも見える共同利用ガイドと、セラピスト用仕様・5〜10分の伴走マニュアルv0.2
- 本人が焦点と終了目安を選び、全画面から終了できる端末内だけの「一緒に見るモード」
- 「困った」後に、動作縮小・判断削減・時間変更・休息・終了を本人が選べる再着手ループ

広告SDK、外部分析SDK、ストリーク、順位、連続未達警告は含めていません。

## 設計文書

Phase 11では、嫌悪に重なる体験を一括せず、分からなさ、評価への怖さ、恥・自責、圧力、退屈、freezeに分けました。気持ちの負担を下げる準備は本人が希望した場合だけ開始プランに置き、緊急性を一般的な動機づけには使いません。詳細は[気持ちの体験に合わせる支援仕様](docs/EMOTIONAL_SUPPORT.md)に記載しています。
...
- [セラピスト伴走仕様 v0.2](docs/THERAPIST_SPEC.md)
- [セラピスト用マニュアル v0.2](docs/THERAPIST_GUIDE.md)
- [本人端末上の伴走モード仕様](docs/SUPPORTED_USE_MODE.md)
- [「困った」後の再着手ループ仕様](docs/STUCK_RETRY_LOOP.md)
